...
const TemplatePicker = ({ onSelect, selectedId }) => (
    ...
    <div className="grid grid-cols-3 gap-3">
      {TEMPLATES.map((t) => (
        <button
          key={t.id}
          onClick={() => onSelect(t)}
          className={`group relative rounded-xl overflow-hidden border-2 transition-all duration-200 hover:scale-105 hover:shadow-lg ${
            selectedId === t.id
        ...
        >
          ...
          <div className={`px-2 py-1.5 flex items-center justify-between border-t ${
            selectedId === t.id ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-gray-100'
          }`}>
            <span className={`text-[11px] font-medium ${selectedId === t.id ? 'text-indigo-600' : 'text-gray-500'}`}>
              {t.name}
            </span>
            {selectedId === t.id && (
              <span className="w-3.5 h-3.5 rounded-full bg-indigo-600 flex items-center justify-center flex-shrink-0">
