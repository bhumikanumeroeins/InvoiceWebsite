      ...
      <div style={{ position: "absolute", top: 348, left: 165, right: 60 }}>
        {items.map((item, i) => (
          <div key={i} style={{ display: "flex", borderBottom: "1px solid #ddd" }}>
            <div style={{ width: "12%", background: dark, color: "#fff", padding: 14 }}>{item.qty}</div>
            <div style={{ width: "38%", padding: 14 }}>{item.description}</div>
            <div style={{ width: "25%", padding: 14, textAlign: "center" }}>{item.unitPrice.toLocaleString("en-IN")}</div>
